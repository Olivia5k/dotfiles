...
block
  (defun th-helm-org-files ()
    (mapcar (lambda (s) (s-chop-suffix ".org" s))
            (directory-files "~/org/" nil ".*\\.org")))

  (defun th-helm-org-visit (file)
    (find-file (format "~/org/%s.org" file)))

  (defvar th-helm-org-sources
    '((name . "org-agenda files")
      (candidates . th-helm-org-files)
      (action . th-helm-org-visit)))

  (defun th-helm-org ()
    (interactive)
    (helm
     :sources th-helm-org-sources
     :prompt "Agenda file: "
     :buffer "*helm-org-agenda*"))

  (global-set-key (kbd "C-x C-o") 'th-helm-org)
#+end_src
** Elisp
#+begin_src emacs-lisp
